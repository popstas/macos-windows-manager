    ...
    pub fn tick(&mut self, seen: &[Seen], index: &BTreeMap<String, SessionRef>, now_ms: u64) {
        ...
        for w in seen {
            ...
            if winners.get(w.title.as_str()) != Some(&w.id) {
                ...
                // давно открытое окно стало бы победителем с непочатым
                // предложением и уехало бы на запомненное место, хотя человек
...
mod tests {
    ...
    #[test]
    fn a_twin_that_outlives_the_winner_stays_where_it_stands() {
        // Проигравший тёзка уходит в `continue` раньше, чем дело дойдёт до
        // слота, — и признак «предложение положено» оставался бы у него
        // нетронутым. Тогда закрытие победителя делало победителем давно
        // открытое окно, и оно уезжало на запомненное место: ровно та драка за
        // перетаскивание, которой правило 2 не велит. Предложение тратится
        // тогда же, когда окно проиграло: оно уже стоит там, где его видит
        // человек.
        let mut t = Tracker::new(1);
        let idx = index(&[("ccfzf", SID)]);
        let mut slots = BTreeMap::new();
        ...
        t.load_slots(slots);
        t.tick(&[], &idx, 1_000);
        // Два окна с одним заголовком: побеждает большее id, оно и расставляется.
        let twins =
            [seen_at(1, "ccfzf", rect(700, 700)), seen_at(9, "ccfzf", rect(700, 700))];
        t.tick(&twins, &idx, 2_000);
        assert_eq!(t.placements(), vec![(9, rect(100, 100))], "слот достался новому окну");
        // Победитель закрылся — проигравший стал победителем. Оно открыто
        // давно, и трогать его поздно.
        t.tick(&[seen_at(1, "ccfzf", rect(700, 700))], &idx, 3_000);
        assert!(t.placements().is_empty(), "переживший тёзку остаётся на месте");
    }

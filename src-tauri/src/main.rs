...
fn run_tracker(
    ...
    loop {
        ...
        // под ним видит. Признак берётся у привязки, а не у номера окна: то же
        // значение уезжает читателю файлом окон, и второй счёт разошёлся бы с
        // ним на первой же правке.
        let minimized_tracked = bound.values().filter(|b| b.minimized).count();
...
mod tests {
    ...
    fn the_tray_counts_minimized_windows_among_the_tracked_ones() {
        ...
        let src = tracker_source();
        assert!(
            src.contains("bound.values().filter(|b| b.minimized).count()"),
